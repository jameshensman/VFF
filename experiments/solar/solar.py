...
m = GPflow.sgpr.SGPR(X, Y, kern=GPflow.kernels.Matern52(1, lengthscales=10.), Z=Z)
m.optimize()
plot(m, axes[0])
...
# VFF
m = VFF.gpr.GPR_1d(X, Y, np.arange(50), a=1590, b=2010, kern=GPflow.kernels.Matern52(1, lengthscales=10.))
m.optimize()
plot(m, axes[3])
