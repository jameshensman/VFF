...
def plot_all(dim):
    ...
    for i, d in enumerate([d_vff, d_sparse]):
        ...
        for j, num_basis in enumerate(num_Basis[dim - 1]):
            ...
            ax.plot(
                df.time[ind_basis],
                -df.marg_lik_x[ind_basis] + df.marg_lik_y[ind_basis],
                bullet[j],
                color=col[i],
            )  # KL
    ...
    for dim in dimensions:
        for a in A:
            ...
            for i, d in enumerate([dv, d_sparse]):
                ...
                # ax.set_ylim((-0.02,1.6))
                ax.semilogx()
                ax.set_title("dimension {}, a = {}".format(dim, np.round(a, 2)))
            # make legend
            points_vff = ax.plot(np.inf, np.inf, "s", color="C0", label="vff")
            points_spa = ax.plot(np.inf, np.inf, "s", color="C1", label="sparse")
            plt.legend()
            for j, num_basis in enumerate(num_Basis[dim - 1]):
                ax.plot(
                    np.inf, np.inf, bullet[j], color="k", label="$M = {}^{}$".format(num_basis, dim)
                )
            # ax.set_xlabel('time (s)')
            plt.legend()
